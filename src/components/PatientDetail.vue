  ...
  <div id="home">
    <div class="flex flex-wrap -mx-3 mb-6">
      <div class="w-full xl:w-1/4 px-3">
        <div class="w-full bg-white border text-blue-400 rounded flex items-center p-6 mb-6 xl:mb-0">
          <svg class="w-8 h-8 fill-current mr-4 hidden lg:block" viewBox="0 0 20 20">
            <path
          ...
          <div class="text-gray-700">
            <p class="font-semibold text-2xl">{{ dataPatient ? dataPatient.patientName : '...' }}</p>
            <p>Document: {{ dataPatient ? dataPatient.patientDocument : '...' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="w-full xl:w-1/4 px-3">
        <div class="w-full bg-white border text-blue-400 rounded-lg flex items-center p-6 mb-6 xl:mb-0">
          ...
          <div class="text-gray-700">
            <p class="font-semibold text-2xl">{{ dataPatient ? dataPatient.agePatient : '...' }} years</p>
            <p>Birthday: {{ dataPatient ? dataPatient.birthdayPatient : '...' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="w-full xl:w-1/4 px-3">
        <div class="w-full bg-white border text-blue-400 rounded flex items-center p-6 mb-6 xl:mb-0">
          ...
          <div class="text-gray-700">
            <p class="font-semibold text-2xl">{{ dataPatient ? dataPatient.planName : '...' }}</p>
            <p>Plan Code: {{ dataPatient ? dataPatient.planCode : '...' }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="w-full xl:w-1/4 px-3">
        <div class="w-full bg-white border text-blue-400 rounded-lg flex items-center p-6">
          ...
          <div class="text-gray-700">
            <p class="font-semibold text-2xl">{{ dataPatient ? dataPatient.lastAppointmentSpecialty : '...' }}</p>
            <p>Last Appointment: {{ dataPatient ? dataPatient.lastAppointmentDate : '...' }}</p>
          </div>
    ...
    <div class="flex flex-col bg-white rounded overflow-hidden">
      <div class="overflow-x-auto">
        <table class="border-collapse w-full">
          ...
          <tbody>
          <template v-for="appointment in paginatedAppointments" :key="appointment.id">
            <tr class="border-b-2 hover:bg-gray-200 cursor-pointer" v-on:click="toggleExpansion(appointment.id)">
              ...
              <td class="px-6 py-4">{{ this.formatDate(appointment.startTime) }}</td>
              <td class="px-6 py-4">{{ appointment.endTime ? this.formatDate(appointment.endTime) : '-' }}</td>
            </tr>
            <tr v-if="opened.includes(appointment.id)">
              <td colspan="6" class="py-3 px-6">
                <div class="flex">
                  <div class="w-full px-6 py-4 bg-gray-300">
                    <label><b>Description:</b></label>
                    <p class="w-full mb-6" rows="3" style="resize: none" disabled>{{ appointment.description }}</p>
                    <label><b>Notes:</b></label>
                    <textarea class="w-full" rows="3" style="resize: none" :disabled="appointment.status !== 'pending'">{{ appointment.notes }}</textarea>
                  </div>
                </div>
              </td>
            </tr>
...
export default {
  ...
  mounted() {
    this.toggleExpansion(parseInt(this.appointmentId))
    this.$forceUpdate();
  },
  watch: {
    patientsResponse: {
  ...
  methods: {
    formatDate(birthDayPatient) {
      const startDate = new Date(birthDayPatient);
    ...
    constructPatientData() {
      ...
      const agePatient = new Date().getFullYear() - new Date(patient.birthday).getFullYear();
      const birthdayPatient = this.formatDate(patient.birthday);
      const planCode = patient.healthSystemId.replace(/(\d{3})(\d{3})(\d{4})/, '$1.$2/$3');
      ...
      if (totalAppointments > 0) {
        ...
        lastAppointmentDate = this.formatDate(lastAppointment.startTime);
      }
      ...
      return {
        ...
        'birthdayPatient': birthdayPatient,
        'planName': planName,
        'planCode': planCode,
